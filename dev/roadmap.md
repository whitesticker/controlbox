...
- [x] **Generic gamepad mapper.** Any controller macOS recognises (`GCExtendedGamepad`) is Add Device → Bluetooth as kind `.gamepad`. `GamepadFamilySession` pools `GamepadSession` / `DualSenseSession`. Several DualSenses and several Xbox pads can each be a sidebar row. Labels follow `GamepadLayout`. Generic USB/Bluetooth **mice** are still later. See [generic-gamepad.md](generic-gamepad.md).
- [ ] **Generic mouse mapper.** A settings pane that maps a generic USB/Bluetooth mouse the same way as a known family. Family sessions are still the add path. Not this version’s HID stack.
...
- [x] **MX Free Spin / Ratchet.** Mouse Settings: segmented **Free Spin** / **Ratchet** plus a SmartShift sensitivity slider. Firmware `0x2111`, fallback `0x2110`. Buttons can map **Switch wheel mode** to toggle it. See [mx-smartshift.md](mx-smartshift.md).
...
- [x] **Window Management rename.** The Window Grab Mac pane is now **Window Management**. Move, resize, throw, organize, shake, and Dock click live there.
...
- [x] **Dock click.** Window Management pane, layered on the native click and state-driven (no double-click): **Switch to Space** goes to the Space that has the window (this display or another) without moving it. **When already in front** picks what one click does when the app is front with a visible window: Minimize window / Hide app / nothing. **Ignored apps** skip Dock click. Listen-only — do not swallow the native click. Modified clicks stay native.
- [ ] **Move all windows here (Dock modifier click).** Shipped in 0.1.47 and pulled in 0.1.48: a modifier-click on a Dock icon gathered that app's windows onto this display and organized them, but only windows already on a current Space or minimized — anything on another Space was skipped, which is the case people want. Bring it back once other-Space windows can be pulled here. Known so far: an AX frame move onto **another display** does relocate an off-Space window to that display's current Space; same-display needs a bounce through a second desktop display; `SLSMoveWindowsToManagedSpace` is dead for other apps' windows without Dock injection; AX minimize/restore returns to the original Space; AX sometimes cannot see off-Space windows at all. See [move-windows-across-spaces.md](move-windows-across-spaces.md).
