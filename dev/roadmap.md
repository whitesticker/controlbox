...
- [x] **Generic gamepad mapper.** Any controller macOS recognises (`GCExtendedGamepad`) is Add Device → Bluetooth as kind `.gamepad`. `GamepadSession` is the base; `DualSenseSession` subclasses it for touchpad + HID battery. Labels follow `GamepadLayout`. Generic USB/Bluetooth **mice** are still later. See [generic-gamepad.md](generic-gamepad.md).
