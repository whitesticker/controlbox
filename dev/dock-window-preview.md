...
- Card click is `DockCardClick` (Core), Dock cards only (switcher cards keep the plain focus): minimized → restore; fullscreen on a current Space → activate only; **Switch to Space** (pane toggle, Card click section) → `WindowSpaces.reveal` for a card on another Space; **When already in front** (Nothing / Minimize / Hide) only when the app was front and the card is its top visible window. Otherwise AX raise as before. Frontmost is read at click time — both panels are non-activating.
