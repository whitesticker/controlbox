...
| Dock click | Listen-only left-mouse tap | Dock AX tile + window list on mouse up | One-shot per click, no double-click timer. Frontmost app is read at mouse-down. Do not swallow the native click. Modified clicks are ignored. |
...
| App switcher previews / App switcher select | Listen-only Command/Tab tap | Window list + stills for the highlighted app; Space switch on Command-up | Tap is up when either toggle is on. Only while the switcher session is up. See [app-switcher-window-preview.md](app-switcher-window-preview.md). |
