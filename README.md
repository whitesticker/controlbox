...
| **Window Management** | Hold a modifier chord and move to drag a window from anywhere; add Shift (by default) to resize with the top-left anchored. Optional throw snaps to a 3×3 map of the screen; optional organize (default Control-Command-O) tiles windows. Optional shake-to-focus hides every other window on this display or all displays. Optional Dock click on top of the native click: **Switch to Space** (go to the Space that has the window, on this display or another; the window does not move), and **When already in front** (one click minimizes the window or hides the app). **Ignored apps** skip Dock click. Trackpad, any mouse, or DualSense. Accessibility must be on. |
