...
- **Night Shift** is its own Mac pane. Off until the pane toggle is on. Then a 24-hour warmth curve drives system Night Shift (`CBBlueLightClient`); Apple’s sunset schedule is paused and restored when the toggle goes off. Optional **Also adjust external brightness** swings external monitors with the curve (cool → brighter, warm → dimmer) by a user slider defaulting to ±10%. Built-in panel is left alone. The catalog lives on the app delegate so the timer survives window close.
