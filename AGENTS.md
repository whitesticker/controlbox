...
- **Display Brightness** is the Mac pane for per-panel brightness (and contrast on supported externals). Optional **One slider for all displays** snapshots each panel’s mix when enabled, fades the per-panel brightness sliders, and scales them from one master (0% all dark, 100% previous brightest at full).
...
- **Night Shift** is its own Mac pane. Off until the pane toggle is on. Then a 24-hour warmth curve drives system Night Shift (`CBBlueLightClient`); Apple’s sunset schedule is paused and restored when the toggle goes off. The catalog lives on the app delegate so the timer survives window close.
...
- **Window grab** is a Mac pane: hold Control and move to drag a window from anywhere; hold Control+Shift and move to resize with the top-left anchored. Works with the trackpad, any mouse, or DualSense. Accessibility must be on. Not gated on an MX Master.
