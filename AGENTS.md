...
- **Window grab** is a Mac pane: hold Control and move to drag a window from anywhere; hold Control+Shift and move to resize with the top-left anchored. Optional **Throw** (default Control+Option, off until the toggle is on) snaps the window under the cursor to a 3×3 map of the pointer’s screen (corners = quarters, edges = halves, center = full). Optional **Organize** (default Control-Command-O, off until the toggle is on) is a recorded shortcut (modifiers plus a key) that tiles visible windows on that screen. Works with the trackpad, any mouse, or DualSense. Accessibility must be on. Not gated on an MX Master. Move, resize, throw, and Display Arrangement cannot share an exact modifier set; Organize cannot reuse Display Arrangement’s number/arrow keys with the same modifiers. The picker alerts and refuses a collision.
- **Dock Previews** is its own Mac pane. Off until the pane toggle is on. Hover a Dock icon that has open windows to see them (including minimized and other Spaces — virtual desktops, not other displays) and click a card to focus that window, or × to close it, or power to quit the app. Apps with no windows stay native — no empty card. Card size is a slider, default 130%. Optional **Show Dock icon names** snapshots and clears pinned `com.apple.dock` `file-label` strings (restore when the toggle is on). Do not `killall Dock`. Always use the flat panel chrome — no callout shape. Hide while a Dock right-click menu is open. Works on whichever display currently has the Dock; place from the icon AX frame, not a hardcoded point. Do not pin Dock auto-hide. Liquid Glass chrome. Catalog lives on the app delegate so hover survives window close. Accessibility finds Dock icons; Screen Recording is optional for thumbnails and is not the Sound audio grant. Hide the panel while Window Grab is held. Do not intercept native Dock clicks. See [docs/dock-window-preview.md](docs/dock-window-preview.md).
...
- With **Allow while Control Box is focused** off, a frontmost Control Box window skips **device** injection (gestures, thumb HID++ scroll, buttons). Window grab is a Mac shortcut and still runs. System-navigation is not a bypass. See `docs/focused-host-still-injects.md`.
...
Start at `docs/README.md`. MX4 feel and architecture: `docs/mx-master-4-pointer-and-haptic.md`. Why Gestures is haptic-only: `docs/haptic-vs-back-gesture.md`. Open work: `docs/todo.md`. Repeating timers and system-API polls: `docs/polling-loops.md`. One issue type per file. Update those when a new failure mode or fix item shows up. Add a row to `polling-loops.md` when you add a loop.
