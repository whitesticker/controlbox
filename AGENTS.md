...
- **Display Arrangement** is its own Mac pane (not brightness). Presets are keyed by the connected external combo plus whether the built-in panel is in the layout. Apply is exact-match only; the editor is a sandbox. Keyboard shortcut is a 3+ modifier chord plus number/arrows, HUD shows the layout snapshot, and that chord cannot match Window Grab move/resize. See DisplayArrangement.swift.
